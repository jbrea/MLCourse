...
mlcode("""
nclust = 2 # nclust is defined by the slider value above => to do
...
""",
"""
nclust = 2 # nclust is defined by the slider value above
...
hc = make_pipeline(StandardScaler(),AgglomerativeClustering(n_clusters = 3, metric = "euclidean", linkage = "complete")).fit(mnist.drop(["class"], axis =1))
hc
""";
eval = false
)
...
"""
hc.predict((mnist.drop(["class"], axis =1))) #dont define : to do
""";
eval = false
)
