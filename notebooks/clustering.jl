...
# ╔═╡ 1b0c0eba-2201-4f15-ba97-a056e3c41ecc
md"# 2. Gaussian Mixture Models"

# ╔═╡ 8d8b1e71-bb22-4a77-b603-c1172d297269
mlcode("",
"""
import numpy as np
import matplotlib.pyplot as plt
...
for i in range(n_components):
    plt.scatter(samples[labels == i, 0], samples[labels == i, 1], label=f'Component {i+1}', alpha=0.6)
plt.xlabel('X1')
plt.ylabel('X2')
plt.title('Synthetic 2D Data from Gaussian Mixture Model')
plt.legend()
plt.grid(True)
plt.show()
"""
...
posterior_probs = gmm.predict_proba(samples)
cluster_assignments = np.argmax(posterior_probs, axis=1)
max_probs = np.max(posterior_probs, axis=1)

plt.figure(figsize=(8, 6))
scatter = plt.scatter(samples[:, 0], samples[:, 1], c=cluster_assignments, cmap='viridis', s=40, alpha=max_probs)
plt.xlabel('X1')
plt.ylabel('X2')
plt.title('GMM Cluster Assignments (Color) & Confidence (Alpha)')
...
md"Although the data was generated by a gaussian mixture model, the algorithm may fail to find the expected three clusters. With a different random state, we may find another result:"

# ╔═╡ 4bc58dad-fa44-46d4-8beb-c6bf7e773069
mlcode("",
"""
gmm = GaussianMixture(n_components=3, covariance_type='full', random_state=16)
gmm.fit(samples)
...
# Plot: color by cluster, intensity by confidence
plt.figure(figsize=(8, 6))
scatter = plt.scatter(samples[:, 0], samples[:, 1], c=cluster_assignments, cmap='viridis', s=40, alpha=max_probs)
plt.xlabel('X1')
plt.ylabel('X2')
...
Number of clusters $( @bind number_of_clust Slider(collect(1:10), show_value=true, default = 4)) (relevant for K-means and hierarchical clustering) 

Minimal number of points $(@bind minpts Slider(collect(1:20), show_value = true, default = 10)) (relevant for DBSCAN)

Radius (epsilon) $(@bind dbscaneps Slider(collect(.2:.2:5), show_value = true, default = 1)) (relevant for DBSCAN)
"""
...
begin
	...
	smiley_kmeans_pred = predict(fit!(machine(KMeans(k = number_of_clust), smiley), verbosity = 0),
		                         smiley)
	smiley_hclust_pred = predict(machine(HierarchicalClustering(k = number_of_clust)), smiley)
	smiley_dbscan_pred = predict(machine(DBSCAN(min_cluster_size = minpts, radius = dbscaneps)), smiley)
end;
