...
let loading = [cos(ϕ), sin(ϕ)]
	...
	annotate!([(1.25, 0, text("ϕ₁ = (ϕ₁₁, ϕ₂₁) ≈ ($(round(loading[1], sigdigits = 2)), $(round(loading[2], sigdigits = 2)))", 9, :red, :left))])
	annotate!([(1.25, .5, text("1/n∑ zᵢ₁² ≈ $(round(mean(abs2, data1'*loading), sigdigits = 3))", 9, :blue, :left))])
	idx = argmax(data1[2, :])
